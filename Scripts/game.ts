...
(function () {
    ...
    interface Enemy {
        ...
        hitBy: number;
        alive: boolean;
        vy: number;             // vertical velocity (leapers)
        nextLeapAt: number;     // next time a leaper springs (leapers)
    }
    ...
    interface Pickup {
        type: "heart" | "crossbow" | "holy" | "scythe";
        x: number;
        y: number;
        bob: number;
        born: number;
    ...
    interface Bolt { x: number; y: number; vx: number; alive: boolean; }
    interface Dust { x: number; y: number; vx: number; vy: number; life: number; max: number; color: string; }
    interface Star { x: number; y: number; r: number; tw: number; par: number; }
    interface Grave { x: number; w: number; h: number; par: number; }
    ...
    interface GameState {
        ...
        popups: Popup[];
        banner: Banner | null;
        shake: number;          // screen-shake magnitude in px, decays to 0
        runToken: string | null; // signed token from /api/run/start, sent on score submit
    }
    ...
    function groundY(): number { return viewH * CONFIG.groundFrac; }
    ...
    function keyDown(e: KeyboardEvent): void {
        ...
        if (["arrowleft", "a"].includes(k)) { input.left = true; heldKeys.add(k); e.preventDefault(); }
        else if (["arrowright", "d"].includes(k)) { input.right = true; heldKeys.add(k); e.preventDefault(); }
        else if (["arrowup", "w", " ", "spacebar"].includes(k) || e.code === "Space") { pressJump(); e.preventDefault(); }
        else if (["j", "f", "x", "enter"].includes(k)) { tryAttack(); e.preventDefault(); }
    }
    function keyUp(e: KeyboardEvent): void {
        const k = e.key.toLowerCase();
        if (["arrowleft", "a"].includes(k)) { heldKeys.delete(k); input.left = heldKeys.has("arrowleft") || heldKeys.has("a"); }
        else if (["arrowright", "d"].includes(k)) { heldKeys.delete(k); input.right = heldKeys.has("arrowright") || heldKeys.has("d"); }
    }
    ...
    function platGap(): number { return 240 + Math.random() * 220; }
    function makePlatform(x: number): Platform {
        const top = groundY() - (96 + Math.random() * 96);
        return { x, y: top, w: 80 + Math.random() * 90 };
    }
    function ensurePlatforms(): void {
        const right = state.camX + viewW + 600;
        const left = state.camX - 600;
        while (state.platCursorR < right) { state.platforms.push(makePlatform(state.platCursorR)); state.platCursorR += platGap(); }
        while (state.platCursorL > left) { state.platCursorL -= platGap(); state.platforms.push(makePlatform(state.platCursorL)); }
    ...
    function spawnPickup(): void {
        ...
        const type: Pickup["type"] = roll < 0.16 ? "heart"
            : roll < 0.46 ? "crossbow"
            : roll < 0.73 ? "scythe"
            : "holy";
        ...
        if (overlap && Math.random() < 0.5) { y = overlap.y - 26; }
    ...
    function tryAttack(): void {
        ...
        for (const e of state.enemies) {
            ...
            if (inX && inY && facingOk) { hitEnemy(e, state.attackId, scythe); hitAny = true; }
        }
        ...
        if (state.boss) {
            ...
            if (inX && facingOk && b.hitBy !== state.attackId) { b.hitBy = state.attackId; damageBoss(scythe ? 3 : 1); hitAny = true; }
        }
        if (!hitAny) { state.combo = 0; Sound.sfx.whiff(); updateHud(); } // whiff drops the combo
    }

    function hitEnemy(e: Enemy, attackId: number, lethal: boolean = false): void {
        e.hitBy = attackId;
        Sound.sfx.stake();
        if (lethal) { e.hp = 1; }   // Scythe one-shots even multi-hit brutes
        e.hp--;
        if (e.hp > 0) { spawnDust(e.x, e.y - e.h * 0.5, 6, "#8a6a9a"); return; }
        e.alive = false;
        state.combo++;
    ...
    function spawnDust(x: number, y: number, n: number, color?: string): void {
        for (let i = 0; i < n; i++) {
    ...
    function update(dt: number): void {
        ...
        if (p.onGround) { p.lastGroundMs = nowMs; }
        ...
        for (const b of state.bolts) {
            ...
            for (const e of state.enemies) {
                if (e.alive && Math.abs(e.x - b.x) < e.w * 0.6 && Math.abs((e.y - e.h * 0.5) - b.y) < e.h * 0.6) {
                    hitEnemy(e, projHitId--); b.alive = false; break;
                }
            }
            if (b.alive && state.boss && Math.abs(state.boss.x - b.x) < state.boss.w * 0.6) { damageBoss(1); b.alive = false; }
        ...
        for (const pk of state.pickups) {
            pk.bob += dt * 4;
            if (Math.abs(pk.x - p.x) < 26 && Math.abs(pk.y - (p.y - p.h * 0.5)) < 44) { pk.taken = true; applyPickup(pk); }
        }
        ...
        if (powerActive) { p._wasCross = true; updateHud(); }
        else if (p._wasCross) { p._wasCross = false; updateHud(); }
    }

    function overlapsPlayer(e: Enemy): boolean {
        const p = state.player;
    ...
    function render(): void {
        ...
        ctx.fillStyle = sky; ctx.fillRect(-OS, -OS, viewW + OS * 2, gy + OS);
        ...
        ctx.beginPath(); ctx.arc(viewW * 0.8, gy * 0.26, 34, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "rgba(11,10,20,0.55)";
        ctx.beginPath(); ctx.arc(viewW * 0.8 + 12, gy * 0.26 - 6, 30, 0, Math.PI * 2); ctx.fill();
        ...
        ground.addColorStop(0, "#27331f"); ground.addColorStop(1, "#121a0e");
        ctx.fillStyle = ground; ctx.fillRect(-OS, gy, viewW + OS * 2, viewH - gy + OS);
        ctx.strokeStyle = "rgba(255,255,255,0.08)";
        ctx.beginPath(); ctx.moveTo(0, gy); ctx.lineTo(viewW, gy); ctx.stroke();

        // Platforms (crypt ledges).
        for (const plat of state.platforms) {
            const x = plat.x - state.camX;
            if (x + plat.w < -40 || x > viewW + 40) { continue; }
        ...
        for (const d of state.dust) {
            ...
            ctx.beginPath(); ctx.arc(d.x - state.camX, d.y, 2 + k * 3, 0, Math.PI * 2); ctx.fill();
        }
        ctx.globalAlpha = 1;
    ...
    function drawEnemy(e: Enemy): void {
        ...
        if (e.def.flying) {
            ...
            ctx.beginPath(); ctx.arc(0, -e.h * 0.5, e.h * 0.45, 0, Math.PI * 2); ctx.fill();
            const wing = Math.sin(nowMs / 60) * 6;
            ctx.beginPath();
            ctx.moveTo(0, -e.h * 0.5);
            ctx.lineTo(-e.w * 0.7, -e.h * 0.5 - wing); ctx.lineTo(-e.w * 0.3, -e.h * 0.3);
            ctx.moveTo(0, -e.h * 0.5);
            ctx.lineTo(e.w * 0.7, -e.h * 0.5 - wing); ctx.lineTo(e.w * 0.3, -e.h * 0.3);
            ctx.fill();
            ctx.fillStyle = "#ff3b3b"; ctx.fillRect(-4, -e.h * 0.55, 2.5, 2.5); ctx.fillRect(2, -e.h * 0.55, 2.5, 2.5);
        } else {
            ...
            if (e.type === "brute" && e.hp < e.def.hp) {
                ctx.fillStyle = "#7a2233"; ctx.fillRect(-e.w / 2, -e.h + 4, e.w * (e.hp / e.def.hp), 3);
            }
        }
        ctx.restore();
    }
    ...
    function drawBoss(b: Boss): void {
        ...
        ctx.fillStyle = "#cdbcd6"; ctx.beginPath(); ctx.arc(0, -b.h + 14, 16, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#ff2020"; ctx.fillRect(-8, -b.h + 10, 5, 5); ctx.fillRect(3, -b.h + 10, 5, 5);
        ctx.restore();
    }
    ...
    function drawPickup(pk: Pickup): void {
        ...
        if (pk.type === "heart") {
            ctx.fillStyle = "#c83b54"; ctx.font = "20px serif"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
            ctx.fillText("❤", 0, 1);
        } else if (pk.type === "crossbow") {
            ...
            ctx.strokeStyle = "#c7ccd2"; ctx.lineWidth = 3;
            ctx.beginPath(); ctx.moveTo(2, -9); ctx.quadraticCurveTo(-9, -9, -8, 0); ctx.stroke();
        } else {
    ...
    function updateHud(): void {
        ...
        if (hudPower) {
            ...
            if (sc > 0) { parts.push("⚔ " + Math.ceil(sc / 1000)); }
    ...
    function requestRunToken(): void {
        fetch("/api/run/start", { method: "POST" })
            ...
            .catch(() => { /* offline / captive weirdness — submit will just be rejected */ });
    }
    ...
    async function loadBoard(period: "all" | "today"): Promise<void> {
        ...
        try {
            ...
            if (resp.ok) { top = await resp.json(); }
    ...
    function isMine(row: Row): boolean {
